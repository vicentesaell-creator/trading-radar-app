...
def obtener_tickers_dowjones():
    try:
        url = "https://en.wikipedia.org/wiki/Dow_Jones_Industrial_Average"
        tablas = pd.read_html(url)
        df = tablas[1] # En el Dow Jones, los componentes suelen estar en la segunda tabla
        column_name = 'Symbol' if 'Symbol' in df.columns else 'Ticker'
        return [str(t).replace('.', '-') for t in df[column_name].tolist()]
    except Exception:
        return []

def ejecutar_escaner_dow():
    print("🚀 Iniciando Super Escáner: Alpha Radar -> ESCANEO EN VIVO DOW JONES...")
    tickers_lista = obtener_tickers_dowjones()
    if not tickers_lista:
        print("❌ No se pudo cargar la lista del Dow Jones.")
        return
        
    print(f"Analizando un total de {len(tickers_lista)} activos del Dow Jones.\n")
    resultados_finales = []
    ...
    if resultados_finales:
        print("🎯 ¡ACTIVOS ENCONTRADOS EN EL DOW JONES! ($5 - $40) 🎯")
        print("=" * 65)
        ...
        print("❌ Ningún activo del Dow Jones superó los filtros estrictos en el rango de $5-$40 hoy.")
